...
struct TypeChecker {
  ...
  void register_builtin_ctors() {
    ...
    if (!env.lookup_ctor("None")) {
      TypePtr opt =
          Type::make_app(Type::make_struct("Option"), {unify.fresh()});
      env.register_ctor("None", opt);
    }
    if (!env.lookup_ctor("Ok")) {
      TypePtr tvar = unify.fresh();
      TypePtr res =
          Type::make_app(Type::make_struct("Result"), {tvar, unify.fresh()});
      env.register_ctor("Ok", Type::make_fn({tvar}, res));
    }
    if (!env.lookup_ctor("Err")) {
      TypePtr tvar = unify.fresh();
      TypePtr res =
          Type::make_app(Type::make_struct("Result"), {unify.fresh(), tvar});
  ...
  void report_warning_at(const ASTNode *node, const std::string &message,
                         int length = 1) {
    const char *filename = node_file(node);
    int line = node_line(node);
    int col = node_col(node);
    set_location_with_column(filename, line, col);
    report_simple_error_with_length(ERROR_LEVEL_WARNING, ERROR_WARNING,
                                    message.c_str(), length);
  }

  void report_warning_at_snippet(const ASTNode *node,
                                 const std::string &message,
                                 const std::string &snippet) {
    report_warning_with_location_and_snippet(message.c_str(), node_file(node),
                                             node_line(node), snippet.c_str());
  }
  ...
  bool is_array_like_type(const TypePtr &type) {
    ...
    return resolved->kind == TypeKind::Array ||
           resolved->kind == TypeKind::FixedArray;
  }

  ArrayParamUsage *find_array_param(std::vector<ArrayParamUsage> &params,
                                    const char *name) {
    if (!name) {
      return nullptr;
    }
  ...
  const char *identifier_name(ASTNode *node) {
    return node && node->type == AST_IDENTIFIER ? node->data.identifier.name
                                                : nullptr;
  }
  ...
  std::string member_expr_snippet(const char *object_name,
  ...
  void scan_array_param_usage(ASTNode *node,
    ...
    switch (node->type) {
    case AST_IDENTIFIER: {
      if (ArrayParamUsage *usage =
              find_array_param(params, node->data.identifier.name)) {
        usage->used = true;
        if (in_return) {
          usage->returned = true;
        }
    ...
    case AST_CALL: {
      ...
      if (func && func->type == AST_MEMBER_ACCESS) {
        ...
        const char *field_name =
            identifier_name(func->data.member_access.field);
        if (ArrayParamUsage *usage = find_array_param(params, object_name)) {
          ...
          if (field_name && strcmp(field_name, "push") == 0) {
            ...
            if (!usage->first_modify) {
              ...
              usage->first_modify_length =
                  member_expr_length(object_name, field_name);
              usage->first_modify_snippet =
                  member_expr_snippet(object_name, field_name);
            }
            if (usage->first_length_read &&
                !usage->warned_modify_after_length) {
              report_warning_at_snippet(
                  usage->first_length_read,
                  "array parameter '" + usage->name +
                      "' is modified after reading its length\n"
                      "  note: the length was read at line " +
                      std::to_string(node_line(usage->first_length_read)) +
                      ", but modifications to the copy won't affect the "
                      "original",
      ...
      if (node->data.call.args &&
        ...
        for (int i = 0; i < count; i++) {
          scan_array_param_usage(
              node->data.call.args->data.expression_list.expressions[i], params,
              in_return);
        }
      }
      break;
    ...
    case AST_MEMBER_ACCESS: {
      ...
      if (ArrayParamUsage *usage = find_array_param(params, object_name)) {
        usage->used = true;
        if (field_name && (strcmp(field_name, "length") == 0 ||
                           strcmp(field_name, "size") == 0)) {
          if (!usage->first_length_read) {
            usage->first_length_read = object ? object : node;
            usage->first_length_read_length =
                member_expr_length(object_name, field_name);
            usage->first_length_read_snippet =
                member_expr_snippet(object_name, field_name);
          }
    ...
    case AST_PROGRAM:
      for (int i = 0; i < node->data.program.statement_count; i++) {
        scan_array_param_usage(node->data.program.statements[i], params,
                               in_return);
      }
      break;
    case AST_ASSIGN:
    ...
    case AST_IF:
      ...
      break;
    case AST_WHILE:
      scan_array_param_usage(node->data.while_stmt.condition, params,
                             in_return);
    ...
    case AST_STRUCT_LITERAL:
      scan_array_param_usage(node->data.struct_literal.fields, params,
                             in_return);
      break;
    case AST_EXPRESSION_LIST:
      for (int i = 0; i < node->data.expression_list.expression_count; i++) {
        scan_array_param_usage(node->data.expression_list.expressions[i],
                               params, in_return);
      }
      break;
    case AST_PRINT:
  ...
  std::vector<ArrayParamUsage> collect_array_params(ASTNode *fn) {
    std::vector<ArrayParamUsage> params;
    if (!fn || !fn->data.function.params ||
        fn->data.function.params->type != AST_EXPRESSION_LIST) {
      return params;
    }
    int count = fn->data.function.params->data.expression_list.expression_count;
    for (int i = 0; i < count; i++) {
      ASTNode *param =
          fn->data.function.params->data.expression_list.expressions[i];
      ...
      usage.name = param->data.assign.left->data.identifier.name
                       ? param->data.assign.left->data.identifier.name
                       : "";
    ...
    for (const auto &usage : params) {
      ...
      if (usage.modified && !usage.returned && usage.first_modify) {
        report_warning_at_snippet(
            usage.first_modify,
            "array parameter '" + usage.name +
                "' is modified but changes are lost\n"
                "  note: array parameters are passed by value (copy)",
            usage.first_modify_snippet);
      }
      if (usage.first_length_read_after_modify && !usage.returned) {
        report_warning_at(
            usage.first_length_read_after_modify,
            "reading array parameter '" + usage.name +
                "' after modifying a copy\n"
                "  note: the array parameter was modified earlier, but the "
                "original caller's array is unchanged");
      }
    }
  }
  ...
  TypeInfo *make_type_info(const TypePtr &type) {
    ...
    case TypeKind::Struct: {
      ...
      if (!name.empty()) {
        ...
        if (si && !si->fields.empty()) {
          ...
          info->params =
              (TypeInfo **)calloc(info->param_count, sizeof(TypeInfo *));
          if (info->params) {
    ...
    case TypeKind::App: {
      ...
      if (info->app_arg_count > 0) {
        info->app_args =
            (TypeInfo **)calloc(info->app_arg_count, sizeof(TypeInfo *));
  ...
  TypePtr type_from_ast(ASTNode *type_node) {
    ...
    case AST_TYPE_POINTER:
      if (type_node->data.pointer_type.element_type) {
        return Type::make_ptr(
            type_from_ast(type_node->data.pointer_type.element_type));
      }
      return Type::make_ptr(unify.fresh());
    case AST_TYPE_LIST:
      return Type::make_array(
          type_node->data.list_type.element_type
              ? type_from_ast(type_node->data.list_type.element_type)
              : unify.fresh());
    case AST_TYPE_FIXED_SIZE_LIST:
      return Type::make_fixed_array(
          type_from_ast(type_node->data.fixed_size_list_type.element_type),
          static_cast<size_t>(type_node->data.fixed_size_list_type.size));
    case AST_TYPE_APP: {
      TypePtr ctor = type_from_ast(type_node->data.type_app.ctor);
      std::vector<TypePtr> args;
      if (type_node->data.type_app.args &&
          type_node->data.type_app.args->type == AST_EXPRESSION_LIST) {
        int count = type_node->data.type_app.args->data.expression_list
                        .expression_count;
        args.reserve(count);
        for (int i = 0; i < count; i++) {
          args.push_back(type_from_ast(type_node->data.type_app.args->data
                                           .expression_list.expressions[i]));
        }
      }
      if (ctor->kind == TypeKind::Struct &&
          ctor->data.struct_data.name == "Fn" && !args.empty()) {
    ...
    case AST_IDENTIFIER:
      ...
      return Type::make_struct(type_node->data.identifier.name
                                   ? type_node->data.identifier.name
                                   : "<anon>");
    case AST_EXPRESSION_LIST: {
      ...
      for (int i = 0; i < cnt; i++) {
        elems.push_back(
            type_from_ast(type_node->data.expression_list.expressions[i]));
      }
  ...
  bool is_numeric(const TypePtr &type) {
    ...
    return t->kind == TypeKind::I8 || t->kind == TypeKind::I32 ||
           t->kind == TypeKind::I64 || t->kind == TypeKind::F32 ||
           t->kind == TypeKind::F64;
  }
  ...
  bool is_valid_vararg_type(const TypePtr &type) {
    ...
    return t->kind == TypeKind::I8 || t->kind == TypeKind::I32 ||
           t->kind == TypeKind::I64 || t->kind == TypeKind::F32 ||
           t->kind == TypeKind::F64 || t->kind == TypeKind::Bool ||
           t->kind == TypeKind::String || t->kind == TypeKind::Ptr ||
           t->kind == TypeKind::Var;
  }
  ...
  bool is_string_compatible(const TypePtr &type) {
    ...
    TypePtr t = unify.apply(type);
    return t->kind == TypeKind::String || t->kind == TypeKind::Ptr ||
           t->kind == TypeKind::Var;
  }
  ...
  TypePtr register_function(ASTNode *fn) {
    ...
    if (fn->data.function.generic_params &&
        fn->data.function.generic_params->type == AST_EXPRESSION_LIST) {
      int cnt = fn->data.function.generic_params->data.expression_list
                    .expression_count;
      for (int i = 0; i < cnt; i++) {
        ASTNode *g = fn->data.function.generic_params->data.expression_list
                         .expressions[i];
    ...
    if (fn->data.function.params &&
        fn->data.function.params->type == AST_EXPRESSION_LIST) {
      int count =
          fn->data.function.params->data.expression_list.expression_count;
      for (int i = 0; i < count; i++) {
        ASTNode *param =
            fn->data.function.params->data.expression_list.expressions[i];
        if (!param) {
    ...
    TypePtr fn_type = Type::make_fn(std::move(params), ret, generic_ids,
                                    fn->data.function.vararg != 0);
  ...
  TypePtr check_program(ASTNode *node) {
    ...
    // Step 1: collect all struct definition nodes.
    std::vector<ASTNode *> struct_nodes;
    for (int i = 0; i < node->data.program.statement_count; i++) {
      ASTNode *stmt = node->data.program.statements[i];
      if (stmt && stmt->type == AST_STRUCT_DEF) {
        struct_nodes.push_back(stmt);
      }
    }

    // Step 2: build complete StructInfo for each struct (fields, generic
    // params)
    std::vector<StructInfo> struct_infos;
    struct_infos.reserve(struct_nodes.size());
    for (ASTNode *stmt : struct_nodes) {
      struct_infos.push_back(build_struct_info(stmt));
    }

    // Step 3: register all structs (makes them visible to each other and to
    // cycle check)
    for (StructInfo &info : struct_infos) {
      env.register_struct(std::move(info));
    }

    // Step 4: check for mutual recursion
    for (ASTNode *stmt : struct_nodes) {
      const char *name =
          stmt->data.struct_def.name ? stmt->data.struct_def.name : "<anon>";
      const StructInfo *si = env.lookup_struct(name);
      if (si) {
        check_struct_cycles(*si);
      }
    }

    // Step 5: compute layout for all structs (requires mutable access)
    for (ASTNode *stmt : struct_nodes) {
      const char *name =
          stmt->data.struct_def.name ? stmt->data.struct_def.name : "<anon>";
      StructInfo *si_mut = const_cast<StructInfo *>(env.lookup_struct(name));
      if (si_mut) {
        compute_struct_layout(*si_mut, env);
      }
    }
  ...
  bool type_references_struct(const TypePtr &t, const std::string &sname) {
    ...
    if (a->kind == TypeKind::App) {
      if (type_references_struct(a->data.app.ctor, sname))
        return true;
      for (const auto &arg : a->data.app.args) {
        if (type_references_struct(arg, sname))
          return true;
      }
    }
    ...
    if (a->kind == TypeKind::Fn) {
      for (const auto &p : a->data.fn.params) {
        if (type_references_struct(p, sname))
          return true;
      }
  ...
  StructInfo build_struct_info(ASTNode *node) {
    StructInfo info;
    ...
    if (node->data.struct_def.generic_params &&
      ...
      int gcount = node->data.struct_def.generic_params->data.expression_list
                       .expression_count;
      for (int i = 0; i < gcount; i++) {
        ASTNode *g = node->data.struct_def.generic_params->data.expression_list
                         .expressions[i];
        if (!g || g->type != AST_IDENTIFIER || !g->data.identifier.name) {
          continue;
        }
    ...
    info.name =
        node->data.struct_def.name ? node->data.struct_def.name : "<anon>";
    info.generic_param_ids = struct_generic_ids;
    if (node->data.struct_def.fields &&
        node->data.struct_def.fields->type == AST_EXPRESSION_LIST) {
      int count =
          node->data.struct_def.fields->data.expression_list.expression_count;
      for (int i = 0; i < count; i++) {
        ASTNode *field =
            node->data.struct_def.fields->data.expression_list.expressions[i];
        ...
        finfo.name = field_name->data.identifier.name
                         ? field_name->data.identifier.name
                         : "<field>";
    ...
    // Direct self-recursion check (no pointer)
  ...
  void check_struct_cycles(const StructInfo &info) {
    // Detect cycles through generic wrappers (App, Array) as well
    std::function<bool(const TypePtr &, const std::string &)>
        type_reaches_struct;
    std::set<std::string> visited; // visited set for has_cycle

    std::function<bool(const std::string &, const std::string &)> has_cycle;
    has_cycle = [&](const std::string &current,
      ...
      for (const auto &sf : si->fields) {
        if (type_reaches_struct(sf.type, target)) {
          return true;
        }
      }
      return false;
    ...
    type_reaches_struct = [&](const TypePtr &t,
                              const std::string &target) -> bool {
      TypePtr a = unify.apply(t);
      if (a->kind == TypeKind::Struct) {
        return has_cycle(a->data.struct_data.name, target);
      }
      if (a->kind == TypeKind::App) {
        if (type_reaches_struct(a->data.app.ctor, target))
          return true;
        for (const auto &arg : a->data.app.args) {
          if (type_reaches_struct(arg, target))
            return true;
        }
      }
      if (a->kind == TypeKind::Array || a->kind == TypeKind::FixedArray) {
        TypePtr elem = (a->kind == TypeKind::Array)
                           ? a->data.array.element
                           : a->data.fixed_array.element;
        return type_reaches_struct(elem, target);
      }
      if (a->kind == TypeKind::Ptr) {
        return false; // pointer breaks the cycle
      }
      return false;
    };
    ...
    for (const auto &f : info.fields) {
      ...
      if (ft->kind == TypeKind::Ptr)
        continue;
      // clear visited before each check to allow independent paths
      visited.clear();
      if (type_reaches_struct(f.type, info.name)) {
        report_semantic_error(
            nullptr, std::string("mutually recursive struct fields must use "
                                 "pointer type (involving '") +
                         info.name + "')");
        return;
      }
    }
  ...
  void check_adt_def(ASTNode *node) {
    ...
    if (is_registered_ctor(name)) {
      ...
      if (payload_count > 0) {
        ...
        TypePtr result =
            Type::make_app(Type::make_struct(base_name ? base_name : "<anon>"),
                           std::move(args));
  ...
  TypePtr check_expression_list(ASTNode *node) {
    ...
    for (int i = 0; i < count; i++) {
      elem_types.push_back(
          check_expr(node->data.expression_list.expressions[i]));
    }
  ...
  TypePtr check_assign(ASTNode *node) {
      ...
      if (annotated && right) {
        try {
          ...
          if (ann_is_num && r_is_num) {
            // ok
          } else if ((resolved_ann->kind == TypeKind::FixedArray &&
                      resolved_r->kind == TypeKind::Array) ||
                     (resolved_ann->kind == TypeKind::Array &&
                      resolved_r->kind == TypeKind::FixedArray)) {
            // ok
          } else if ((resolved_ann->kind == TypeKind::String &&
                      resolved_r->kind == TypeKind::Ptr) ||
                     (resolved_ann->kind == TypeKind::Ptr &&
                      resolved_r->kind == TypeKind::String)) {
            // ok
          } else if (resolved_ann->kind == TypeKind::FixedArray &&
                     resolved_r->kind == TypeKind::Ptr) {
            ...
            report_type_error(node,
                              std::string("type mismatch in let binding: ") +
                                  ex.what());
          }
        }
      }
    ...
    if (left && left->type == AST_IDENTIFIER && right &&
    ...
    if (left && left->type == AST_IDENTIFIER && right &&
      ...
      if (obj && obj->type == AST_IDENTIFIER && obj->data.identifier.name &&
        ...
        std::string key = std::string(obj->data.identifier.name) + "." +
                          std::string(fld->data.identifier.name);
    ...
    if (left && left->type == AST_IDENTIFIER) {
      const char *name =
          left->data.identifier.name ? left->data.identifier.name : "";
      bool is_decl = node->data.assign.is_declaration != 0;
      if (is_decl) {
        if (!env.declare_value(name, rtype,
                               left->mutability == MUTABILITY_MUTABLE, false)) {
        ...
        if (!val) {
          report_semantic_error(
              node, std::string("assignment to undeclared variable: ") + name);
        } else if (!val->is_mutable) {
          report_semantic_error(
              node, std::string("cannot assign to immutable variable '") +
                        name +
          ...
          try {
            ...
            if (!((lresolved->kind == TypeKind::String &&
                   rresolved->kind == TypeKind::Ptr) ||
                  (lresolved->kind == TypeKind::Ptr &&
                   rresolved->kind == TypeKind::String))) {
              report_type_error(node, ex.what());
            }
          }
        }
  ...
  TypePtr check_binop(ASTNode *node) {
          ...
          if (is_nil_or_zero) {
            ...
            if (resolved->kind == TypeKind::App && resolved->data.app.ctor &&
              ...
              const std::string &base =
                  resolved->data.app.ctor->data.struct_data.name;
    ...
    if (op == OP_ADD) {
      ...
      bool has_string_operand =
          (rl->kind == TypeKind::String || rr->kind == TypeKind::String);
      if (has_string_operand && is_string_compatible(lhs) &&
          is_string_compatible(rhs)) {
        try {
          if (rl->kind == TypeKind::Var)
            unify.unify(lhs, builtin_string);
          if (rr->kind == TypeKind::Var)
            unify.unify(rhs, builtin_string);
        } catch (...) {
        }
        return node_types[node] = builtin_string;
  ...
  TypePtr check_unaryop(ASTNode *node) {
    ...
    case OP_DEREF: {
      ...
      if (resolved->kind == TypeKind::I32 || resolved->kind == TypeKind::I64 ||
          ...
          resolved->kind == TypeKind::String) {
        report_type_error(node, "cannot dereference non-pointer type (use '@' "
  ...
  TypePtr check_if(ASTNode *node) {
    ...
    bool narrow_else = false;
    std::string scrutinee_name;
    TypePtr then_payload_type;
    TypePtr else_payload_type;

    auto detect_match = [&](BinOpType op, ASTNode *lhs, ASTNode *rhs) {
      ...
      if (lhs && lhs->type == AST_IDENTIFIER && lhs->data.identifier.name &&
          (is_builtin_ctor(lhs->data.identifier.name) ||
           vix_adt_ctor_index(lhs->data.identifier.name) >= 0)) {
        ctor_node = lhs;
        scrutinee_node = rhs;
      } else if (rhs && rhs->type == AST_IDENTIFIER &&
                 rhs->data.identifier.name &&
                 (is_builtin_ctor(rhs->data.identifier.name) ||
                  vix_adt_ctor_index(rhs->data.identifier.name) >= 0)) {
        ctor_node = rhs;
        scrutinee_node = lhs;
      } else if (lhs && lhs->type == AST_NIL && rhs &&
                 rhs->type == AST_IDENTIFIER) {
        is_nil_cmp = true;
        scrutinee_node = rhs;
      } else if (rhs && rhs->type == AST_NIL && lhs &&
                 lhs->type == AST_IDENTIFIER) {
        is_nil_cmp = true;
        scrutinee_node = lhs;
      }
      if (!ctor_node && !is_nil_cmp) {
        if (lhs && lhs->type == AST_MEMBER_ACCESS && rhs &&
            rhs->type == AST_IDENTIFIER &&
            (is_builtin_ctor(rhs->data.identifier.name) ||
             vix_adt_ctor_index(rhs->data.identifier.name) >= 0)) {
          ASTNode *field = lhs->data.member_access.field;
          if (field && field->type == AST_IDENTIFIER &&
              strcmp(field->data.identifier.name, "0") == 0) {
            ctor_node = rhs;
            scrutinee_node = lhs->data.member_access.object;
          }
        } else if (rhs && rhs->type == AST_MEMBER_ACCESS && lhs &&
                   lhs->type == AST_IDENTIFIER &&
                   (is_builtin_ctor(lhs->data.identifier.name) ||
                    vix_adt_ctor_index(lhs->data.identifier.name) >= 0)) {
          ASTNode *field = rhs->data.member_access.field;
          if (field && field->type == AST_IDENTIFIER &&
              strcmp(field->data.identifier.name, "0") == 0) {
            ctor_node = lhs;
            scrutinee_node = rhs->data.member_access.object;
          }
        }
      }

      if ((ctor_node || is_nil_cmp) && scrutinee_node &&
          scrutinee_node->type == AST_IDENTIFIER) {
        scrutinee_name = scrutinee_node->data.identifier.name
                             ? scrutinee_node->data.identifier.name
                             : "";
        if (!scrutinee_name.empty()) {
          ...
          if (is_nil_cmp) {
            if (resolved->kind == TypeKind::App && resolved->data.app.ctor &&
              ...
              if (base == "Option" && resolved->data.app.args.size() == 1) {
                then_payload_type = resolved->data.app.args[0];
                if (is_ne) {
                  narrow_then = true;
                }
              } else if (base == "Result" &&
                         resolved->data.app.args.size() == 2 && is_ne) {
                then_payload_type = resolved->data.app.args[0];
                narrow_then = true;
              }
            }
            if (resolved->kind == TypeKind::Ptr && is_ne) {
              then_payload_type = resolved;
              narrow_then = true;
            }
          } else {
              ...
              if (base == "Option" && resolved->data.app.args.size() == 1) {
                if (is_ne && strcmp(ctor_name, "None") == 0) {
                  ...
                  narrow_then = true;
                         ...
                         resolved->data.app.args.size() == 2) {
                if (is_ne) {
                  // For !=, then branch holds the opposite constructor's
                  // payload
                  if (ctor_name && strcmp(ctor_name, "Ok") == 0) {
                    then_payload_type =
                        resolved->data.app.args[1]; // then is Err
                    else_payload_type =
                        resolved->data.app.args[0]; // else is Ok
                  } else if (ctor_name && strcmp(ctor_name, "Err") == 0) {
                    then_payload_type =
                        resolved->data.app.args[0]; // then is Ok
                    else_payload_type =
                        resolved->data.app.args[1]; // else is Err
                  } else {
                    then_payload_type = resolved->data.app.args[0];
                  }
                  narrow_then = true;
                  ...
                  if (strcmp(ctor_name, "Ok") == 0) {
                    ...
                    then_payload_type = resolved->data.app.args[1];
                    narrow_then = true;
                  }
                }
              } else {
                ASTNode *payload_node =
                    vix_adt_ctor_payload_type_node(ctor_name);
                if (payload_node) {
                  then_payload_type = type_from_ast(payload_node);
                  narrow_then = !is_ne;
                }
              }
          ...
          // For other != cases, narrow_else might have been set already or we
          // rely on later logic.
          if (then_payload_type && !narrow_then && is_ne) {
            // This branch handles non-Result != patterns (Option != Some, etc.)
            narrow_else = true;
            if (ctor_node && ctor_node->data.identifier.name) {
              const char *ctor = ctor_node->data.identifier.name;
              if (resolved->kind == TypeKind::App && resolved->data.app.ctor &&
                  resolved->data.app.ctor->kind == TypeKind::Struct) {
                const std::string &base =
                    resolved->data.app.ctor->data.struct_data.name;
                if (base == "Option" && strcmp(ctor, "Some") == 0 &&
                    resolved->data.app.args.size() == 1) {
                  else_payload_type =
                      resolved->data.app.args[0]; // else is Some payload
                }
              }
            }
            if (!else_payload_type) {
              else_payload_type = then_payload_type; // fallback
            }
          }
        }
      }
    ...
    if (node->data.if_stmt.condition &&
      ...
      BinOpType op = node->data.if_stmt.condition->data.binop.op;
      if (op == OP_EQ || op == OP_NE) {
        detect_match(op, node->data.if_stmt.condition->data.binop.left,
                     node->data.if_stmt.condition->data.binop.right);
      }
    }

    auto saved_payloads = match_payloads;
    auto saved_field_types = match_payload_field_types;

    if (narrow_then && then_payload_type) {
      match_payloads[scrutinee_name] = then_payload_type;
      match_payload_field_types[scrutinee_name + ".1"] = then_payload_type;
      match_payload_field_types[scrutinee_name + ".0"] = builtin_i32;
    }

    TypePtr then_t = check_block(node->data.if_stmt.then_body, true);

    match_payloads = std::move(saved_payloads);
    match_payload_field_types = std::move(saved_field_types);

    if (narrow_else && else_payload_type) {
      match_payloads[scrutinee_name] = else_payload_type;
      match_payload_field_types[scrutinee_name + ".1"] = else_payload_type;
      match_payload_field_types[scrutinee_name + ".0"] = builtin_i32;
    }

    TypePtr else_t = node->data.if_stmt.else_body
                         ? check_block(node->data.if_stmt.else_body, true)
                         : builtin_void;
  ...
  TypePtr check_for(ASTNode *node) {
    if (!node->data.for_stmt.var ||
        node->data.for_stmt.var->type != AST_IDENTIFIER) {
    ...
    env.declare_value(node->data.for_stmt.var->data.identifier.name, var_type,
                      false, false);
  ...
  TypePtr check_member(ASTNode *node) {
    ...
    if (numeric_field) {
      // Auto-dereference pointers to reach the underlying type, but limit loop
      // iterations.
      TypePtr current = resolved;
      int deref_count = 0;
      while (current->kind == TypeKind::Ptr &&
             deref_count < 10) { // safety limit
        current = unify.apply(current->data.ptr.pointee);
        deref_count++;
      }
      int idx = atoi(field);

      if (current->kind == TypeKind::Array) {
        return set_member_type(current->data.array.element);
      }
      if (current->kind == TypeKind::FixedArray) {
        return set_member_type(current->data.fixed_array.element);
      }
      if (current->kind == TypeKind::Tuple) {
        if (idx >= 0 && idx < (int)current->data.tuple.elements.size()) {
          return set_member_type(current->data.tuple.elements[idx]);
        }
        report_semantic_error(node, "tuple index out of bounds");
        return set_member_type(unify.fresh());
      }
      if (current->kind == TypeKind::App && current->data.app.ctor &&
          current->data.app.ctor->kind == TypeKind::Struct) {
        const std::string &base = current->data.app.ctor->data.struct_data.name;
        if (idx == 0) {
          return set_member_type(builtin_i32);
        }
        if (idx == 1) {
          ASTNode *objectNode = node->data.member_access.object;
          if (objectNode && objectNode->type == AST_IDENTIFIER &&
              objectNode->data.identifier.name) {
            std::string key = std::string(objectNode->data.identifier.name) +
                              "." + std::string(field);
            auto it = match_payload_field_types.find(key);
            if (it != match_payload_field_types.end()) {
          ...
          if (base == "Option" && current->data.app.args.size() >= 1) {
            report_semantic_error(
                node, "accessing payload of Option without pattern matching");
            return set_member_type(unify.fresh());
          }
          if (base == "Result" && current->data.app.args.size() >= 2) {
            report_semantic_error(
                node, "accessing payload of Result without pattern matching");
            return set_member_type(unify.fresh());
          }
          ...
          if (payload_node) {
            report_semantic_error(
                node, "accessing payload of ADT without pattern matching");
            return set_member_type(unify.fresh());
          }
          // Unknown custom ADT, but index 1 is still accessible only through
          // narrowing
          report_semantic_error(node,
                                std::string("cannot access payload of ADT '") +
                                    base + "' without pattern matching");
          return set_member_type(unify.fresh());
        }
        // idx >= 2
        report_semantic_error(node, "index out of bounds for ADT");
        return set_member_type(unify.fresh());
      }

      // If still unresolved, check match_payload_field_types (may be set for
      // non-ADT)
      {
        ASTNode *objectNode = node->data.member_access.object;
        if (objectNode && objectNode->type == AST_IDENTIFIER &&
      ...
      report_type_error(node, "numeric member access on unsupported type");
      return set_member_type(unify.fresh());
    }

    // length/size: auto-dereference pointer to access array/string
    if (strcmp(field, "length") == 0 || strcmp(field, "size") == 0) {
      TypePtr res_target = unify.apply(target);
      // If it's a pointer, look through one level
      if (res_target->kind == TypeKind::Ptr) {
        res_target = unify.apply(res_target->data.ptr.pointee);
      }
      if (res_target->kind == TypeKind::Array ||
          res_target->kind == TypeKind::FixedArray ||
          res_target->kind == TypeKind::String) {
        return set_member_type(builtin_i32);
      }
    }
    ...
    if (strcmp(field, "push") == 0) {
      ...
      return set_member_type(Type::make_fn({elem}, builtin_void));
    }
    ...
    if (!info) {
      ...
      report_semantic_error(node, "unknown struct: " +
                                      resolved->data.struct_data.name);
      return set_member_type(unify.fresh());
    }
    for (const auto &f : info->fields) {
      if (f.name == field) {
        if (!app_args.empty() && !info->generic_param_ids.empty()) {
          ...
          for (size_t gi = 0;
               gi < info->generic_param_ids.size() && gi < app_args.size();
               ++gi) {
          ...
          auto rewrite = [&](const TypePtr &t, auto &self) -> TypePtr {
            ...
            case TypeKind::FixedArray:
              return Type::make_fixed_array(
                  self(a->data.fixed_array.element, self),
                  a->data.fixed_array.size);
            case TypeKind::App: {
              std::vector<TypePtr> args;
              args.reserve(a->data.app.args.size());
              ...
              return Type::make_app(self(a->data.app.ctor, self),
                                    std::move(args));
            }
            case TypeKind::Fn: {
              std::vector<TypePtr> params;
              ...
              return Type::make_fn(
                  std::move(params), self(a->data.fn.ret, self),
                  a->data.fn.generic_param_ids, a->data.fn.vararg);
            }
            case TypeKind::Tuple: {
              std::vector<TypePtr> elems;
  ...
  TypePtr check_struct_literal(ASTNode *node) {
    ...
    if (node->data.struct_literal.type_name->type == AST_IDENTIFIER) {
      name = node->data.struct_literal.type_name->data.identifier.name;
               ...
               node->data.struct_literal.type_name->data.type_app.ctor->type ==
                   AST_IDENTIFIER) {
      name = node->data.struct_literal.type_name->data.type_app.ctor->data
                 .identifier.name;
    }
    if (!name) {
      report_semantic_error(node, "struct literal without name");
    ...
    if (node->data.struct_literal.type_name->type == AST_TYPE_APP &&
        !info->generic_param_ids.empty()) {
      ASTNode *arg_list =
          node->data.struct_literal.type_name->data.type_app.args;
      if (arg_list && arg_list->type == AST_EXPRESSION_LIST) {
        ...
        for (int i = 0; i < n; i++) {
          TypePtr concrete =
              type_from_ast(arg_list->data.expression_list.expressions[i]);
    ...
    auto subst_type = [&](const TypePtr &t, auto &self_ref) -> TypePtr {
      ...
      if (a->kind == TypeKind::App) {
        ...
        return Type::make_app(self_ref(a->data.app.ctor, self_ref),
                              std::move(new_args));
      }
      if (a->kind == TypeKind::Fn) {
        ...
        return Type::make_fn(std::move(new_params),
                             self_ref(a->data.fn.ret, self_ref));
      }
      return a;
    };
    ...
    if (node->data.struct_literal.fields &&
        node->data.struct_literal.fields->type == AST_EXPRESSION_LIST) {
      int count = node->data.struct_literal.fields->data.expression_list
                      .expression_count;
      for (int i = 0; i < count; i++) {
        ASTNode *field = node->data.struct_literal.fields->data.expression_list
                             .expressions[i];
        if (!field || field->type != AST_ASSIGN) {
          continue;
        }
        ...
        for (const auto &f : info->fields) {
          if (f.name == fname) {
            found = true;
            initialized_fields.insert(f.name);
            TypePtr ft = check_expr(field_expr);
            TypePtr field_type =
                generic_subst.empty() ? f.type : subst_type(f.type, subst_type);

            std::function<void(const TypePtr &, const TypePtr &)>
                coerce_tuple_to_array;
            coerce_tuple_to_array = [&](const TypePtr &expected,
                                        const TypePtr &actual) {
              TypePtr re = unify.apply(expected);
              TypePtr ra = unify.apply(actual);
              if (re && re->kind == TypeKind::Array && ra &&
                  ra->kind == TypeKind::Tuple) {
                TypePtr elem = re->data.array.element;
                for (const auto &te : ra->data.tuple.elements) {
                  coerce_tuple_to_array(elem, te);
                }
              } else if (re && re->kind == TypeKind::FixedArray && ra &&
                         ra->kind == TypeKind::Ptr) {
                // okay
              } else {
    ...
    for (const auto &f : info->fields) {
      if (initialized_fields.find(f.name) == initialized_fields.end()) {
        report_semantic_error(node, std::string("missing field '") + f.name +
                                        "' in struct literal");
      }
    }
    ...
    if (node->data.struct_literal.type_name->type == AST_TYPE_APP) {
      ...
      return node_types[node] =
                 Type::make_app(Type::make_struct(name), std::move(args));
    }
    return node_types[node] = Type::make_struct(name);
  }

  TypePtr instantiate_fn(const TypePtr &fn_type,
    ...
    std::unordered_set<int> generic_id_set(
        resolved->data.fn.generic_param_ids.begin(),
        resolved->data.fn.generic_param_ids.end());
    std::unordered_map<int, TypePtr> mapping;
    for (size_t i = 0;
         i < resolved->data.fn.generic_param_ids.size() && i < type_args.size();
         ++i) {
    ...
    auto rewrite = [&](const TypePtr &t, auto &rewrite_ref) -> TypePtr {
      ...
      TypePtr a = t;
      if (generic_id_set.find(t->kind == TypeKind::Var ? t->data.var.id : -1) ==
          generic_id_set.end()) {
        a = unify.apply(t);
      }
      if (a->kind == TypeKind::Var) {
      ...
      case TypeKind::Array:
        return Type::make_array(
            rewrite_ref(a->data.array.element, rewrite_ref));
      case TypeKind::FixedArray:
        return Type::make_fixed_array(
            rewrite_ref(a->data.fixed_array.element, rewrite_ref),
            a->data.fixed_array.size);
      case TypeKind::App: {
        std::vector<TypePtr> args;
        args.reserve(a->data.app.args.size());
        ...
        return Type::make_app(rewrite_ref(a->data.app.ctor, rewrite_ref),
                              std::move(args));
      }
      case TypeKind::Fn: {
        std::vector<TypePtr> params;
        ...
        return Type::make_fn(std::move(params),
                             rewrite_ref(a->data.fn.ret, rewrite_ref),
                             a->data.fn.generic_param_ids, a->data.fn.vararg);
      }
      case TypeKind::Tuple: {
        std::vector<TypePtr> elems;
  ...
  TypePtr check_call(ASTNode *node) {
    if (node->data.call.func &&
        node->data.call.func->type == AST_MEMBER_ACCESS) {
      ...
      if (objectNode && fieldNode && fieldNode->type == AST_IDENTIFIER &&
          fieldNode->data.identifier.name) {
        std::string methodName(fieldNode->data.identifier.name);
        std::string typeName;
        TypePtr objType = check_expr(objectNode);

        if (objectNode->type == AST_IDENTIFIER &&
            objectNode->data.identifier.name) {
          std::string objName(objectNode->data.identifier.name);
          if (env.lookup_struct(objName) ||
              vix_is_adt_definition(objName.c_str())) {
            typeName = objName;
          }
        }
        if (typeName.empty()) {
        ...
        if (!typeName.empty()) {
          const char *mangledFunc =
              vix_lookup_impl_method(typeName.c_str(), methodName.c_str());
          if (mangledFunc) {
            TypePtr fnType = env.lookup_value(mangledFunc)
                                 ? env.lookup_value(mangledFunc)->type
                                 : TypePtr(nullptr);
            if (fnType && fnType->kind == TypeKind::Fn) {
              // Freshen the method type to preserve parametric polymorphism
              fnType = freshen_type(fnType);
              ...
              int actualArgs = node->data.call.args
                                   ? node->data.call.args->data.expression_list
                                         .expression_count
                                   : 0;
              int expectedArgs = isStaticCall ? totalParams : totalParams - 1;
              if (actualArgs != expectedArgs) {
                report_semantic_error(node, std::string("method '") +
                                                methodName + "' expects " +
              ...
              // Unify self parameter with receiver, allowing automatic
              // referencing
              if (!isStaticCall && totalParams > 0) {
                TypePtr selfParam = fnType->data.fn.params[0];
                TypePtr resolvedSelf = unify.apply(selfParam);
                TypePtr resolvedObj = unify.apply(objType);
                bool success = false;
                try {
                  unify.unify(selfParam, objType);
                  success = true;
                } catch (...) {
                  // If self is a pointer and obj is a value, auto-reference
                  if (resolvedSelf->kind == TypeKind::Ptr) {
                    TypePtr objPtr = Type::make_ptr(resolvedObj);
                    try {
                      unify.unify(selfParam, objPtr);
                      success = true;
                    } catch (...) {
                    }
                  }
                ...
                if (!success) {
                  report_type_error(
                      node, std::string("method receiver type mismatch"));
                }
              }

              int paramOffset = isStaticCall ? 0 : 1;
              for (int i = 0; i < actualArgs && i + paramOffset < totalParams;
                   i++) {
                ASTNode *arg =
    ...
    // Constructor-like calls that appear as plain function calls (Some, None,
    // Ok, Err)
    if (node->data.call.func && node->data.call.func->type == AST_IDENTIFIER) {
      ...
      if (cname && strcmp(cname, "Some") == 0) {
        ...
        if (node->data.call.args &&
          ...
          arg_t = check_expr(
              node->data.call.args->data.expression_list.expressions[0]);
        } else {
          report_semantic_error(node, "Some(...) expects one argument");
        }
        return node_types[node] =
                   Type::make_app(Type::make_struct("Option"), {arg_t});
      }
      ...
      if (cname && strcmp(cname, "Ok") == 0) {
        ...
        return node_types[node] = Type::make_app(Type::make_struct("Result"),
                                                 {arg_t, unify.fresh()});
      }
      ...
      if (ctor_type) {
        ...
        if (resolved_ctor->kind == TypeKind::Fn) {
          ...
          if (!resolved_ctor->data.fn.vararg && expected != actual) {
            report_semantic_error(node, std::string("constructor '") + cname +
                                            "' arity mismatch");
          }
          if (node->data.call.args &&
              node->data.call.args->type == AST_EXPRESSION_LIST) {
            for (size_t i = 0; i < actual && i < expected; i++) {
              TypePtr arg_t = check_expr(
                  node->data.call.args->data.expression_list.expressions[i]);
              try {
    ...
    bool has_type_args =
        node->data.call.type_args &&
        node->data.call.type_args->type == AST_EXPRESSION_LIST &&
    ...
    if (resolved->kind == TypeKind::Fn) {
      if (!has_type_args) {
        resolved = freshen_type(resolved);
      }
      ...
      if (node->data.call.type_args &&
        ...
        int count =
            node->data.call.type_args->data.expression_list.expression_count;
        for (int i = 0; i < count; i++) {
          type_args.push_back(type_from_ast(
              node->data.call.type_args->data.expression_list.expressions[i]));
        }
      ...
      for (size_t i = 0; i < actual && i < expected; i++) {
        ...
        if (param_type->kind == TypeKind::Ptr &&
            arg_type->kind == TypeKind::String) {
          continue;
        }
        if (param_type->kind == TypeKind::Ptr &&
            arg_type->kind == TypeKind::FixedArray) {
          continue;
        }
        if (param_type->kind == TypeKind::Ptr &&
      ...
      if (fn_type->data.fn.vararg) {
        for (size_t i = expected; i < actual; i++) {
          ...
          if (!is_valid_vararg_type(arg_type)) {
            report_type_error(node, "cannot pass type '" +
                                        unify.pretty(arg_type) +
                                        "' to variadic function parameter");
          }
        }
      }
  ...
  TypePtr check_global(ASTNode *node) {
    ...
    TypePtr declared_type =
        type_node ? type_from_ast(type_node) : TypePtr(nullptr);
    ...
    TypePtr var_type =
        declared_type ? unify.apply(declared_type) : unify.apply(init_type);

    if (identifier && identifier->type == AST_IDENTIFIER) {
      const char *name = identifier->data.identifier.name;
      if (name) {
        if (!env.declare_value(name, var_type,
                               identifier->mutability == MUTABILITY_MUTABLE,
                               false)) {
          report_semantic_error(node, std::string("redefinition: ") + name);
        }
      }
  ...
  TypePtr check_function(ASTNode *node) {
    ...
    if (node->data.function.generic_params &&
      ...
      int count = node->data.function.generic_params->data.expression_list
                      .expression_count;
      const auto &ids = fn_type->data.fn.generic_param_ids;
      for (int i = 0; i < count; i++) {
        ASTNode *g = node->data.function.generic_params->data.expression_list
                         .expressions[i];
        if (!g || g->type != AST_IDENTIFIER || !g->data.identifier.name) {
          continue;
        }
    ...
    if (node->data.function.params &&
        node->data.function.params->type == AST_EXPRESSION_LIST) {
      int count =
          node->data.function.params->data.expression_list.expression_count;
      for (int i = 0; i < count; i++) {
        ASTNode *param =
            node->data.function.params->data.expression_list.expressions[i];
        if (!param) {
          continue;
        }
        if (param->type == AST_ASSIGN && param->data.assign.left &&
            param->data.assign.left->type == AST_IDENTIFIER) {
          TypePtr ptype = type_from_ast(param->data.assign.right);
          env.declare_value(param->data.assign.left->data.identifier.name,
                            ptype, param->mutability == MUTABILITY_MUTABLE,
                            false);
    ...
    if (!(node->data.function.is_extern &&
          node->data.function.body == nullptr)) {
      TypePtr resolved_ret = unify.apply(ret_type);
      TypePtr resolved_body = unify.apply(body_type);
      bool skip_unify = false;
      if (resolved_ret->kind == TypeKind::Void &&
          resolved_body->kind != TypeKind::Void) {
        bool has_explicit_return =
            find_return_node(node->data.function.body) != nullptr;
      ...
      if (!skip_unify) {
        try {
          ...
          if (resolved_body->kind == TypeKind::Ptr &&
              resolved_body->data.ptr.pointee) {
            try {
          ...
          if (!auto_deref_ok && resolved_ret->kind == TypeKind::Ptr &&
              resolved_ret->data.ptr.pointee) {
            try {
